...
function buscarRepositorios1() {
    ...
    fetch(`https://api.github.com/users/${usuario}/repos`)
        .then(response => response.json().then(detalhes => {
            console.log(detalhes)
            document.getElementById('nomeRepositorio1').innerHTML = detalhes[2].name;
            document.getElementById('descricaoRepositorio1').innerHTML = 'Descrição: ' + detalhes[2].description;
            document.getElementById('repositorioAtualizadoEm1').innerHTML = 'Atualizado em: ' + detalhes[2].updated_at;
...
function detalhesGit() {
    ...
    fetch(`https://api.github.com/users/${usuario}`)
        .then(response => response.json().then(data => {
            ...
            maisRepositorios.href = `${data.html_url}?tab=repositories`;
